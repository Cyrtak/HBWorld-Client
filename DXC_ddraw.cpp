...
BOOL DXC_ddraw::bInit(HWND hWnd)
{
 ...
 int            iS, iD;
	...
	if( m_bFullMode == TRUE )
	{
		...
		//SetRect(&m_rcFlipping, 0, 0, 800, 600); // our fictitious sprite bitmap is 
	}
	else
	{
		...
		//SetRect(&m_rcFlipping, cx-400, cy-300, cx+400, cy+300); // our fictitious sprite bitmap is 
...
BOOL DXC_ddraw::SetDisplayMode(HWND hWnd, int m_iX, int m_iY, int m_iColour)
{
	HRESULT        ddVal;
	DDSURFACEDESC2 ddsd;
	...
	//ddVal = m_lpDD4->SetDisplayMode(800, 600, 16,0,0);
	ddVal = m_lpDD4->SetDisplayMode(m_iX, m_iY, m_iColour,0,0);
	if (ddVal != DD_OK) return FALSE;
	memset( (VOID *)&ddsd, 0, sizeof(ddsd) );
	ddsd.dwSize = sizeof( ddsd );
	ddsd.dwFlags = DDSD_CAPS | DDSD_BACKBUFFERCOUNT;
	...
	ddVal = m_lpFrontB4->GetAttachedSurface(&ddscaps, &m_lpBackB4flip);
	if (ddVal != DD_OK) return FALSE;
	SetRect(&m_rcFlipping, 0, 0, m_iX, m_iY); // our fictitious sprite bitmap is 
	//SetRect(&m_rcFlipping, 0, 0, 800, 600); // our fictitious sprite bitmap is 
	m_bFullMode = TRUE;

	InitFlipToGDI(hWnd);
	m_lpBackB4 = pCreateOffScreenSurface(m_iX, m_iY);
	//m_lpBackB4 = pCreateOffScreenSurface(800, 600);
	if (m_lpBackB4 == NULL) return FALSE;

	// Pre-draw background surface
	m_lpPDBGS = pCreateOffScreenSurface(m_iX+32, m_iY+32);
	//m_lpPDBGS = pCreateOffScreenSurface(800+32, 600+32);
	if (m_lpPDBGS == NULL) return FALSE;
