...
// Snoopy: Added all 351 creature
	...
	m_stFrame[93][DEF_OBJECTSTOP].m_sMaxFrame		= 7;
	m_stFrame[93][DEF_OBJECTMOVE].m_sFrameTime		= 90;
	m_stFrame[93][DEF_OBJECTATTACK].m_sFrameTime	= 100;
	m_stFrame[93][DEF_OBJECTATTACK].m_sMaxFrame		= 7;
	m_stFrame[93][DEF_OBJECTDAMAGE].m_sFrameTime	= 100;
	m_stFrame[93][DEF_OBJECTDAMAGE].m_sMaxFrame		= 7;
	m_stFrame[93][DEF_OBJECTDYING].m_sFrameTime		= 180;
	m_stFrame[93][DEF_OBJECTDYING].m_sMaxFrame		= 15+3;
	...
	m_stFrame[95][DEF_OBJECTATTACK].m_sFrameTime	= 120;
	m_stFrame[95][DEF_OBJECTATTACK].m_sMaxFrame		= 7;
	m_stFrame[95][DEF_OBJECTDAMAGE].m_sFrameTime	= 120;
	m_stFrame[95][DEF_OBJECTDAMAGE].m_sMaxFrame		= 3 +4;
	m_stFrame[95][DEF_OBJECTDYING].m_sFrameTime		= 100;
	m_stFrame[95][DEF_OBJECTDYING].m_sMaxFrame		= 9;
	...
	m_stFrame[96][DEF_OBJECTATTACK].m_sFrameTime	= 60;
	...
	m_stFrame[97][DEF_OBJECTATTACK].m_sFrameTime	= 60;
	...
	m_stFrame[98][DEF_OBJECTATTACK].m_sFrameTime	= 60;
...
{int dX, dY;
	...
	// air wyvern
	if (m_pData[dX+1][dY+1].m_sOwnerType == 94) return FALSE;
	if (m_pData[dX+1]  [dY].m_sOwnerType == 94) return FALSE;
	if (m_pData[dX+1][dY-1].m_sOwnerType == 94) return FALSE;
	if (m_pData  [dX][dY+1].m_sOwnerType == 94) return FALSE;
	if (m_pData  [dX]  [dY].m_sOwnerType == 94) return FALSE;
	if (m_pData  [dX][dY-1].m_sOwnerType == 94) return FALSE;
	if (m_pData[dX-1][dY+1].m_sOwnerType == 94) return FALSE;
	if (m_pData[dX-1]  [dY].m_sOwnerType == 94) return FALSE;
	if (m_pData[dX-1][dY-1].m_sOwnerType == 94) return FALSE;
	...
	if (m_pData[dX+1][dY+1].m_sOwnerType == 92) return FALSE;
	if (m_pData[dX+1]  [dY].m_sOwnerType == 92) return FALSE;
	if (m_pData[dX+1][dY-1].m_sOwnerType == 92) return FALSE;
	if (m_pData  [dX][dY+1].m_sOwnerType == 92) return FALSE;
	if (m_pData  [dX]  [dY].m_sOwnerType == 92) return FALSE;
	if (m_pData  [dX][dY-1].m_sOwnerType == 92) return FALSE;
	if (m_pData[dX-1][dY+1].m_sOwnerType == 92) return FALSE;
	if (m_pData[dX-1]  [dY].m_sOwnerType == 92) return FALSE;
	if (m_pData[dX-1][dY-1].m_sOwnerType == 92) return FALSE;

	if (m_pData[dX+1][dY+1].m_sOwnerType == 93) return FALSE;
	if (m_pData[dX+1]  [dY].m_sOwnerType == 93) return FALSE;
	if (m_pData[dX+1][dY-1].m_sOwnerType == 93) return FALSE;
	if (m_pData  [dX][dY+1].m_sOwnerType == 93) return FALSE;
	if (m_pData  [dX]  [dY].m_sOwnerType == 93) return FALSE;
	if (m_pData  [dX][dY-1].m_sOwnerType == 93) return FALSE;
	if (m_pData[dX-1][dY+1].m_sOwnerType == 93) return FALSE;
	if (m_pData[dX-1]  [dY].m_sOwnerType == 93) return FALSE;
	if (m_pData[dX-1][dY-1].m_sOwnerType == 93) return FALSE;

	if (m_pData[dX+1][dY+1].m_sOwnerType == 97) return FALSE;
	if (m_pData[dX+1]  [dY].m_sOwnerType == 97) return FALSE;
	if (m_pData[dX+1][dY-1].m_sOwnerType == 97) return FALSE;
	if (m_pData  [dX][dY+1].m_sOwnerType == 97) return FALSE;
	if (m_pData  [dX]  [dY].m_sOwnerType == 97) return FALSE;
	if (m_pData  [dX][dY-1].m_sOwnerType == 97) return FALSE;
	if (m_pData[dX-1][dY+1].m_sOwnerType == 97) return FALSE;
	if (m_pData[dX-1]  [dY].m_sOwnerType == 97) return FALSE;
	if (m_pData[dX-1][dY-1].m_sOwnerType == 97) return FALSE;
...
BOOL __fastcall CMapData::bSetOwner(WORD wObjectID, int sX, int sY, int sType, int cDir, short sAppr1, short sAppr2, short sAppr3, short sAppr4, int iApprColor, int iStatus, char * pName, short sAction, short sV1, short sV2, short sV3, int iPreLoc, int iFrame, char cFlag)
{int   iX, iY, dX, dY;
 int   iChatIndex, iAdd;
 char  cTmpName[12];
 DWORD dwTime;
...
EXIT_SEARCH_LOOP:;
	...
	{	m_pData[dX][dY].m_wObjectID   = wObjectID;
		...
		if (sType >= 10) m_pData[dX][dY].m_cFlag = 0;
		else m_pData[dX][dY].m_cFlag			= cFlag;
				...
				if ( (m_pData[dX][dY].m_cOwnerAction == DEF_OBJECTATTACK) ) { //3 00498685
					...
					case 105:
						...
						{	if  ( m_pGame->bHasHeroSet(m_pData[dX][dY].m_sAppr3, m_pData[dX][dY].m_sAppr4, m_pData[dX][dY].m_sOwnerType) == 1) // Warr hero set
							...
							} else if  ( m_pGame->bHasHeroSet(m_pData[dX][dY].m_sAppr3, m_pData[dX][dY].m_sAppr4, m_pData[dX][dY].m_sOwnerType) == 3) // Warr godly set
							{	m_pGame->bAddNewEffect(247, m_sPivotX + dX, m_sPivotY + dY
									, m_sPivotX + dX , m_sPivotY + dY, 0, 1);
							}
						...
						{	if ((m_pData[dX][dY].m_sAppr2 & 0xF000) != 0) // not Peace mode
							{	if (m_pData[dX][dY].m_sV3 != 1) // autre que corp à corp
								{	
									int iColor = (m_pData[dX][dY].m_iApprColor & 0xF0000000) >> 28;

									if (m_pData[dX][dY].m_sV3 == 3 || m_pData[dX][dY].m_sV3 == 29) // Daggers use colour (above) for visual improvement
										m_pGame->bAddNewEffect(m_pData[dX][dY].m_sV3, m_sPivotX + dX, m_sPivotY + dY
											...
											, 0, m_pData[dX][dY].m_sOwnerType, iColor);
									else
										m_pGame->bAddNewEffect(m_pData[dX][dY].m_sV3, m_sPivotX + dX, m_sPivotY + dY
											, m_sPivotX + dX + m_pData[dX][dY].m_sV1, m_sPivotY + dY + m_pData[dX][dY].m_sV2
					...
					default:
						...
						{	if  (m_pData[dX][dY].m_sV3 == 2) // Arrow flying...
							...
							/*else if  (m_pData[dX][dY].m_sV3 == 8) // Dagger flying...
							{	m_pGame->bAddNewEffect(2, m_sPivotX + dX, m_sPivotY + dY
										, m_sPivotX + dX + m_pData[dX][dY].m_sV1
										, m_sPivotY + dY + m_pData[dX][dY].m_sV2
										, 0, m_pData[dX][dY].m_sOwnerType*1000);
				...
				{	switch (m_pData[dX][dY].m_sOwnerType) {
					...
					case 46: // TK
						...
						{	if (m_pData[dX][dY].m_sV2 == -1)
								...
								iSoundIndex = 6;
							else if (((m_pData[dX][dY].m_sV2 >= 20) && (m_pData[dX][dY].m_sV2 <= 39)) || (m_pData[dX][dY].m_sV2 >= 50))
								iSoundIndex = 6;
							else if ((m_pData[dX][dY].m_sV2 >= 40) && (m_pData[dX][dY].m_sV2 < 50))
								iSoundIndex = 7;
							else iSoundIndex = 5;
					...
					case 105:
						...
						{	m_pGame->PlaySound('C', 16, sDist, lPan);
							...
							{	m_pGame->bAddNewEffect(242, m_sPivotX + dX, m_sPivotY + dY
									, m_sPivotX + dX , m_sPivotY + dY, 0, 1);
							} else if (m_pGame->bHasHeroSet(m_pData[dX][dY].m_sAppr3, m_pData[dX][dY].m_sAppr4, m_pData[dX][dY].m_sOwnerType) == 4) // mage godly set
...
BOOL CMapData::bSetTrap(BOOL bStatus, short sX, short sY, char cItemColor)
{int dX, dY;
 int sAbsX, sAbsY, sDist;
	if ((sX < m_sPivotX) || (sX >= m_sPivotX + MAPDATASIZEX) ||
		(sY < m_sPivotY) || (sY >= m_sPivotY + MAPDATASIZEY))
	...
	m_pData[dX][dY].m_bIsTrapped = bStatus;

	return TRUE;
}

BOOL CMapData::bAddGuildHall(short sX, short sY)
{int dX, dY;
 int sAbsX, sAbsY, sDist;
	if ((sX < m_sPivotX) || (sX >= m_sPivotX + MAPDATASIZEX) ||
		(sY < m_sPivotY) || (sY >= m_sPivotY + MAPDATASIZEY))
	...
	m_pData[dX][dY].m_bIsGuildHallLocation = TRUE;

	return TRUE;
}

BOOL CMapData::bAddGuildHallUpgradeLocation(short sX, short sY)
{int dX, dY;
 int sAbsX, sAbsY, sDist;
	if ((sX < m_sPivotX) || (sX >= m_sPivotX + MAPDATASIZEX) ||
		(sY < m_sPivotY) || (sY >= m_sPivotY + MAPDATASIZEY))
	...
	m_pData[dX][dY].m_bIsGuildHallUpgradeLocation = TRUE;
