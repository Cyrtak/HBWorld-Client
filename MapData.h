...
public:
	...
	BOOL __fastcall bSetOwner(WORD wObjectID, int sX, int sY, int sType, int cDir, short sAppr1, short sAppr2, short sAppr3, short sAppr4, int iApprColor, int iStatus, char * pName, short sAction, short sV1, short sV2, short sV3, int iPreLoc = 0, int iFrame = 0, char cFlag = 0);
	...
	BOOL bSetTrap(BOOL bStatus, short sX, short sY, char cItemColor = 0);
	int  iObjectFrameCounter(char * cPlayerName, short sViewPointX, short sViewPointY);

	BOOL bAddGuildHall(short sX, short sY);
	BOOL bAddGuildHallUpgradeLocation(short sX, short sY);
