	...
	*wp = (WORD)(dwSize + 3);
	...
	if (cKey != NULL  && cKey != 0) {
		for (i = 0; i < (int)(dwSize); i++) {
			...
			//m_pSndBuffer[3+i] = (char)( m_pSndBuffer[3+i] ^ (cKey ^ (dwSize - (i+1))) );
			m_pSndBuffer[3+i] = (char)( m_pSndBuffer[3+i] ^ (cKey ^ (dwSize - (i+2))) );
		}
	}
 ...
 char cKey;
	...
	*pMsgSize = (*wp) - 3;	
	dwSize    = (*wp) - 3;
		...
		wsprintf(debugmsg, "Size: %i", dwSize);
		Debug(debugmsg);

	// v.14 : m_pSndBuffer +3 dwSize± cKey
	if (cKey != NULL) {
		wsprintf(debugmsg, "Starting decryption");
		Debug(debugmsg);
		for (i = 0; i < (int)(dwSize); i++) {
			//m_pRcvBuffer[3+i] = (char)( m_pRcvBuffer[3+i] ^ (cKey ^ (dwSize - (i+1))) );
			m_pRcvBuffer[3+i] = (char)( m_pRcvBuffer[3+i] ^ (cKey ^ (dwSize - (i+2))) );
			m_pRcvBuffer[3+i] -= (i ^ cKey);
		}
	}

	wsprintf(debugmsg, "Full data: %i %i %i %i %i %i %i %i %i", m_pRcvBuffer[0],m_pRcvBuffer[1],m_pRcvBuffer[2],m_pRcvBuffer[3],m_pRcvBuffer[4],m_pRcvBuffer[5], m_pRcvBuffer[6], m_pRcvBuffer[7], m_pRcvBuffer[8]);
		Debug(debugmsg);

	return (m_pRcvBuffer + 3);
}

void XSocket::Debug(char * cMsg)
{
 FILE * pFile;
 char cBuffer[512];
 SYSTEMTIME SysTime;
